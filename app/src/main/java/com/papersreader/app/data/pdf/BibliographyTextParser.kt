...
object BibliographyTextParser {
    ...
    // "Alpha" bracket-code style entries ("[FB81] Martin A Fischler and Robert C Bolles. ...",
    // as generated by natbib's alpha/plainnat styles) — each entry starts at the left margin with
    // its own "[CODE]" marker, same shape as the numbered style but with author-initials+year
    // instead of a plain number. See InlineCitationDetector's identical pattern for what a code
    // looks like (kept in sync manually — small enough that sharing it isn't worth the coupling).
    private val codedEntryMarkerRegex = Regex("(?m)^\\s*\\[\\p{Lu}[\\p{L}]{0,5}(?:\\+\\s?)?\\d{2}[a-z]?\\]\\s*")
    ...
    fun splitEntries(bibliographyText: String): List<ParsedReference> {
        val candidates = listOf(
            ...
            codedEntryMarkerRegex.findAll(bibliographyText).toList(),
        )
