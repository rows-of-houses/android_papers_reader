...
/**
 * A single entry a citation marker resolves to — a printed bracket number, an author+year pair
 * matched against the bibliography by content, or an "alpha" bracket code (author initials +
 * 2-digit year, e.g. "[FB81]", as used by natbib's alpha style) matched by exact code text.
 */
sealed class CitationKey {
    ...
    data class Coded(val code: String) : CitationKey()

    /** What the citation dialog's title shows for this key. */
    val displayLabel: String get() = when (this) {
        is Numbered -> index.toString()
        is AuthorYear -> "$surname, $year"
        is Coded -> code
    }
}
...
object InlineCitationDetector {
    ...
    // One "alpha" code: a capitalized letter, up to 5 more letters, an optional "+" (with
    // optional surrounding space, e.g. "DJC+ 21"), 2 digits, and an optional disambiguating
    // letter suffix ("MF23a", "MF23b") — the same role as author-year's "2018a"/"2018b". The
    // space is deliberately only reachable *after* a "+" — a bare, unconditional optional space
    // before the digits would also match ordinary two-digit-numbered prose like "Table 12" or
    // "Section 19" as if it were a citation code.
    private const val CODED_ENTRY = "\\p{Lu}[\\p{L}]{0,5}(?:\\+\\s?)?\\d{2}[a-z]?"
    // Citations often carry a locator alongside the code(s) — "[Hub64, Section 5-7]", "[Smith99,
    // p. 12]" — so only the *leading* comma-separated run of valid codes is captured; anything
    // after that up to the closing bracket is locator text, not another code, and is ignored
    // rather than making the whole bracket fail to match at all (which is what silently dropped
    // this exact "[Hub64, Section 5-7]" citation before this was added).
    private val codedListRegex = Regex("\\[($CODED_ENTRY(?:\\s*,\\s*$CODED_ENTRY)*)(?:,[^\\]]*)?\\]")
    ...
    fun detect(words: List<PdfWord>): List<InlineCitation> {
        ...
        while (i < words.size) {
            ...
            if (direct != null) {
                citations += InlineCitation(listOf(word.rect), direct, direct.joinToString(", ") { it.displayLabel })
                i++
                continue
            }
    ...
    private fun mergeBracketGroup(words: List<PdfWord>, startIndex: Int): Pair<InlineCitation, Int>? {
        ...
        while (j < words.size && j - startIndex < MAX_MERGE_WORDS) {
            ...
            if (next.text.contains(']')) {
                val keys = parseMarker(builder.toString()) ?: return null
                rects += currentLineRect
                val citation = InlineCitation(rects, keys, keys.joinToString(", ") { it.displayLabel })
    ...
    private fun parseMarker(text: String): List<CitationKey>? {
        rangeRegex.find(text)?.let { m ->
            ...
            return (start..end).map { CitationKey.Numbered(it) }
        }
        singleOrListRegex.find(text)?.let { m ->
            val indices = m.groupValues[1].split(",").mapNotNull { it.trim().toIntOrNull() }
            return indices.map { CitationKey.Numbered(it) }
        }
        // "Alpha" bracket style ("[FB81]", "[Men14, Men15, LM17]", "[DJC+ 21]") — author initials
        // plus a 2-digit year, as generated by natbib's `alpha`/`plainnat` bibliography styles.
        // A trailing "+" (optionally followed by whitespace, as PDF text extraction sometimes
        // renders it) marks "et al." for a paper with more authors than fit in the code. Spaces
        // are stripped from the captured code so it matches consistently regardless of exactly
        // where PDFBox happened to split it into separate words.
        codedListRegex.find(text)?.let { m ->
            return m.groupValues[1].split(",").map { CitationKey.Coded(it.trim().replace(" ", "")) }
        }
        return null
    }
}
...
/**
 * Matches a [CitationKey.Coded] against a bibliography entry's raw text — entries in this style
 * are split on (and so keep) the very same "[CODE]" marker as their own leading text, so matching
 * is just comparing that leading code, once whitespace differences (e.g. "[DJC+ 21]" vs however
 * this exact entry's own extraction happened to space it) are normalized away.
 */
object CodedMatcher {
    private val leadingCode = Regex("^\\[([^\\]]{1,12})\\]")

    fun matches(referenceText: String, code: String): Boolean {
        val entryCode = leadingCode.find(referenceText.trimStart())?.groupValues?.get(1)?.replace(" ", "") ?: return false
